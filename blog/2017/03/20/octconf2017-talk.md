---
title: "Talk at the OctConf 2017 at CERN"
author: "Kai T. Ohlhus"
date: "2017-03-20"
date-modified: "2026-08-22"
categories:
  - octave
  - talk
draft: false
---

The slides of today's talk at the
[OctConf2017](https://wiki.octave.org/OctConf_2017)
at [CERN](https://indico.cern.ch/event/609833/timetable/#20170320.detailed)
are available [here](https://github.com/octave-de/OctConf2017).

The talk was about GNU Octave's
[publish](https://docs.octave.org/interpreter/XREFpublish.html)
and
[grabcode](https://docs.octave.org/interpreter/XREFgrabcode.html)
functions, that I implemented last year and showed some application in
combination with the
[MediaWiki action API](/blog/2017/03/10/getting-to-know-the-mediawiki-api.html).
